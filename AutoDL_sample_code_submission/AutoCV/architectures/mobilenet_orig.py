...
class MobileNetV2(nn.Module):
        ...
        self.conv1d = torch.nn.Sequential(
            skeleton.nn.Split(OrderedDict([
                ...
                ('deep', torch.nn.Sequential(
                    ...
                    torch.nn.Conv1d(self.last_channel, self.last_channel,
                                    kernel_size=5, stride=1, padding=2, bias=False),
                    torch.nn.BatchNorm1d(self.last_channel),
                    torch.nn.ReLU(inplace=True),
                ))
            ])),
    ...
    def forward_image(self, x):
        x = self.features(x)
        # log("x_features_size: {} {} {} {}".format(x.size(0), x.size(1), x.size(2), x.size(3)))
        x = x.mean([2, 3])
        # log("x_mean_size: {} {}".format(x.size(0), x.size(1)))
        x = self.classifier(x)
        # log("x_classifier_size: {} {}".format(x.size(0), x.size(1)))
        return x
