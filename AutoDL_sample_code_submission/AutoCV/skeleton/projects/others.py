...
def get_tf_resize(height, width, times=1, min_value=0.0, max_value=1.0):
    def preprocessor(tensor):
        ...
        '''
        log("tensor_origin_shape: {}".format(tensor.get_shape()))

        if times != in_times or times > 1:
            # resize time axis using NN (to select frame \wo interpolate)
            # tensor_first = tf.expand_dims(tensor[0], 0)
            # tensor_last = tf.expand_dims(tensor[-1], 0)
            # tensor = tf.concat([tensor_first, tensor_last], axis=0)

            # tensor_new = tensor[int(times // 2)]  # single image
            # tensor_new = tf.expand_dims(tensor_new, 0)
            # start_time = int(times // 2)
            # delta = int(times // 2)
            # for i in range(start_time + 1, start_time + delta):
            #     tensor_cur = tf.expand_dims(tensor[i], 0)
            #     tensor_new = tf.concat([tensor_new, tensor_cur], axis=0)

            # tensor = tensor[delta: delta + times]
            # tensor = tensor_new

            start_time = int(times)
            delta = int(times // 2)
            # tensor = tensor[start_time: start_time + delta]
            # tensor = tensor[: start_time]

            tensor_first = tf.expand_dims(tensor[3], 0)
            # tensor_mid = tf.expand_dims(tensor[int(times_min // 4)], 0)
            # tensor_mid_mid = tf.expand_dims(tensor[int(times_min // 2)], 0)
            tensor_mid_last = tf.expand_dims(tensor[-10], 0)
            tensor_mid_last_last = tf.expand_dims(tensor[-7], 0)
            tensor_last = tf.expand_dims(tensor[-3], 0)

            tensor = tf.concat([tensor_first, tensor_mid_last, tensor_mid_last_last, tensor_last], axis=0)

            log("tensor_shape: {}".format(tensor.get_shape()))
        '''
