#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time:    2020/1/30 11:54
# @Author:  Mecthew

import tensorflow as tf
import numpy as np
from tools import log, ohe2cat
...
class TFModel(object):
    ...
    def init_model(self,
        ...
        self.vocabulary_size = num_features
        ...
        self.ft_w_embed = self.get_token_embeddings(self.vocabulary_size, self.embedding_size, zero_pad=False,
                                                    name='w_embed')
        ...
        with tf.variable_scope('FT', reuse=tf.AUTO_REUSE):
            ...
            W1 = tf.get_variable(shape=[num_filters_total, 128], dtype=tf.float32, name='des_w1',
                                 initializer=tf.contrib.layers.xavier_initializer())
            b1 = tf.get_variable(shape=[128], dtype=tf.float32, name='des_b1',
                                 initializer=tf.contrib.layers.xavier_initializer())
            x1 = tf.nn.xw_plus_b(self.h_pool_flat, W1, b1, name="dense1")
            x1 = tf.nn.relu(x1)

            W = tf.get_variable(shape=[128, self.num_classes], dtype=tf.float32, name='des_w',
                                initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable(shape=[self.num_classes], dtype=tf.float32, name='des_b',
                                initializer=tf.contrib.layers.xavier_initializer())
    ...
    def get_token_embeddings(self, vocab_size, num_units, zero_pad=True, name='shared_weight_matrix'):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            embeddings = tf.get_variable(name + '_tag_weight_mat', dtype=tf.float32, shape=(vocab_size, num_units),
                                         initializer=tf.contrib.layers.xavier_initializer())
            if zero_pad:
    ...
    def fit(self, train_x, train_y, validation_data_fit, epochs, cur_model_run_loop, batch_size=64, **kwargs):
